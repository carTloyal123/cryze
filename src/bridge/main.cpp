// main.cpp — Wyze doorbell bridge: one-shot H.264 stream capture
//
// Flow: auth → SDK init → subscribe → LAN poll/inject → AV link → stream → exit
//
// Modes:
//   --stdout     Write raw H.264 to stdout (for go2rtc exec pipe)
//   --output F   Write to file (default: logs/frames.h264)
//   --duration N Seconds to run (0 = indefinite)
...
int main(int argc, char** argv) {
    ...
    // stdout mode: dup fd 1 for H.264 output, redirect fd 1 to stderr
    if (use_stdout) {
        duration = 0;
    ...
    {
        const char* sw = std::getenv("SUBSCRIBE_WAIT");
        int sub_wait = sw ? std::atoi(sw) : 20;
