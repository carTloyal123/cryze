...
def write_go2rtc_streams(registry, overlay: bool = False) -> None:
    """Write per-device streams to go2rtc.yaml before go2rtc starts.

    overlay=False (default): bridge exec'd directly — zero overhead, unchanged.
    overlay=True:            go2rtc reads from wyze-overlay container TCP port.
    """
    streams_block = ""
    for info in registry.devices:
        if overlay:
            source = (f"exec:nc 127.0.0.1 {info.overlay_port}"
                      f"#video=h264#killsignal=2#killtimeout=15")
        else:
...
"""
    ...
    log.info("Wrote %d stream(s) to %s (overlay=%s)",
             len(registry.devices), GO2RTC_CFG, overlay)
    for info in registry.devices:
        log.info("  %s → %s", info.stream_name, info.mac)
        if overlay:
            log.info("    raw_port=%d  overlay_port=%d", info.raw_port, info.overlay_port)


def register_go2rtc_streams(registry, api_url: str = "http://127.0.0.1:1984") -> None:
    """No-op — streams written to YAML before go2rtc starts."""
    pass
...
def main() -> None:
    ...
    overlay_enabled = os.environ.get("STREAM_OVERLAY", "0") in ("1", "true", "yes")
    if overlay_enabled:
        log.info("STREAM_OVERLAY=1 — overlay mode active (wyze-overlay container required)")
    ...
    for d in registry.devices:
        ...
        if overlay_enabled:
            log.info("      [overlay] raw=:%d  out=:%d", d.raw_port, d.overlay_port)
    log.info("=" * 60)
    log.info("")
    ...
    if overlay_enabled:
        ...
        for d in registry.devices:
            try:
                _ck.create_connection(("127.0.0.1", d.overlay_port), timeout=1).close()
            except OSError:
                log.warning(
                    "STREAM_OVERLAY=1 but wyze-overlay not listening on :%d for %s — "
                    "run: docker compose --profile overlay up",
                    d.overlay_port, d.mac)
