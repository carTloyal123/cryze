...
def register_go2rtc_streams(registry, api_url: str = "http://127.0.0.1:1984") -> None:
    """Register one go2rtc stream per device via the REST API.

    Retries until go2rtc is ready (up to 30s).
    Stream name: camera_{mac_clean}
    Source:      exec:/work/scripts/run_bridge.sh --device {mac}#video=h264#...
    """
    # Wait for go2rtc API readiness
    for attempt in range(30):
        try:
            with urllib.request.urlopen(f"{api_url}/api", timeout=2):
                break
        except Exception:
            if attempt == 0:
                log.info("Waiting for go2rtc API...")
            time.sleep(1)
    else:
        log.error("go2rtc API not reachable after 30s — streams not registered")
        return
    ...
    for info in registry.devices:
        ...
        body   = json.dumps({"sources": [source]}).encode()
        req    = urllib.request.Request(
            f"{api_url}/api/streams/{info.stream_name}",
            data=body, method="PUT",
            headers={"Content-Type": "application/json"})
        try:
            with urllib.request.urlopen(req, timeout=5):
                pass
            log.info("  Stream registered: %s → %s", info.stream_name, info.mac)
        except Exception as e:
            log.error("  Failed to register %s: %s", info.stream_name, e)
